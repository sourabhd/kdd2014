...
#    sio.savemat('WV', {'WV_FV': FV, 'WV_Labels': labels, 'WV_tr_idx': train_idx, 'WV_te_idx': test_idx, 'WV_words': words})
    ...
    WV2 = sio.loadmat('WV')
    FV = WV2['WV_FV']
    labels = WV2['WV_Labels'].ravel()
    train_idx = WV2['WV_tr_idx'].ravel()
    test_idx = WV2['WV_te_idx'].ravel()
    words = WV2['WV_words']
    ...
    # linearsvc_params = {'penalty': 'l2', 'loss': 'l2', 'dual': True, 'tol': 0.0001, 'C': 1.0, 'multi_class': 'ovr', 'fit_intercept': True, 'intercept_scaling': 1, 'class_weight': None, 'verbose': 2, 'random_state': None}
   # gbrt_ params = {'n_estimators': 500, 'max_depth': 50, 'min_samples_split': 1, 'learning_rate': 0.01, 'loss': 'ls'}
    M = Learner(FV, labels)
    # P = M.fit_and_predict(train_idx, test_idx, LogisticRegression(), params=logit_params)
    # linearsvc = LinearSVC(penalty='l2', loss='l2', dual=True, tol=0.0001, C=1.0, multi_class='ovr', fit_intercept=True, intercept_scaling=1, class_weight=None, verbose=2, random_state=None)
    rf_params = {'n_estimators': [10, 100, 1000], 'max_depth': [2, 5, 10, 20], 'oob_score': [True], 'verbose': [1]}

    # P = M.fit_and_predict(train_idx, test_idx, linearsvc, params=linearsvc_params)
    P = M.fit_validate_and_predict(train_idx, test_idx, RandomForestClassifier(), rf_params)

    ofile = open('word_embed_rf.csv', 'w')
    ofile.write("projectid,is_exciting\n")
    k = 0
    for i in test_idx:
            ofile.write("%s,%f\n" % (epo.iloc[i]['projectid'], P[k][1]))
